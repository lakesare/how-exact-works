...
\setbeamertemplate{title page}{%
  \vskip2.4cm%
  {\large\inserttitle}\par\vspace{4pt}%
  {\footnotesize\color{inkghst}\insertsubtitle}\par\vspace{10pt}%
...
% --- Metadata ---
\title{How {\color{inkfaint}`exact?{}`} Works}
\subtitle{Discrimination Trees and SolveByElim in Lean 4}
...
\begin{frame}{Global Search: Linear Search}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.14\textwidth}
      \includegraphics[width=\linewidth]{kim_morrison}
    \end{column}
...
\begin{frame}[label=navleanmathlib]{Discrimination Trees: Lean and Mathlib}
  ...
  \begin{tabular}{@{}m{2cm}m{2.7cm}m{4cm}@{}}
    \textbf{Used By} & \textbf{What's Stored} & \textbf{DiscrTree $\alpha$} \\
    \toprule
    ...
    \code{simp} & \code{@[simp]} & \code{DiscrTree SimpTheorem} \\
